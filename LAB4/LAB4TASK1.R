age = c(55,57,56,52,51,59,58,53,59,55,60,60,60,60,52,55,56,51,60,
        52,54,56,52,57,54,56,58,53,53,50,55,51,57,60,57,55,51,50,57,58)

age_table <- data.frame(
  "Staff Age" = names(table(age_factor)),
  "Total number of staff" = as.vector(table(age_factor))
)

print(age_table, row.names = FALSE)

range_table <- data.frame(
  "Age Range" = names(table(age_range)),
  "Total number of staff" = as.vector(table(age_range))
)

print(range_table, row.names = FALSE)
